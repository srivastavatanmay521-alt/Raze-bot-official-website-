...
/* ═══════════════ Extend ripple to nav/footer/logo taps ═══════════════ */
document.querySelectorAll('nav a, .mobile-nav a, .footer-col a, .footer-social a, .nav-logo, .boot-skip').forEach(el => {
  el.classList.add('ripple-host');
  el.addEventListener('pointerdown', function (e) {
    const rect = el.getBoundingClientRect();
...
/* ═══════════════ Global touch glow (fires on real touch taps anywhere) ═══════════════ */
...
/* ═══════════════ 3D tilt on touch devices (module cards, hero logo, stat/partner cards) ═══════════════ */
(function () {
  ...
  document.querySelectorAll('[data-tilt]').forEach(card => {
    ...
    card.addEventListener('touchmove', (e) => {
      ...
      card.classList.add('touch-tilting');
      if (raf) cancelAnimationFrame(raf);
      raf = requestAnimationFrame(() => {
        card.style.transform = `translateY(-4px) rotateX(${(-py * maxTilt * 2).toFixed(2)}deg) rotateY(${(px * maxTilt * 2).toFixed(2)}deg)`;
      });
    }, { passive: true });

    const resetTilt = () => {
      card.classList.remove('touch-tilting');
      card.style.transform = '';
    };
